...
def seleccionar_usuario():
    ...
    if not usuarios:
        print("❌ No hay usuarios disponibles en la base de datos.")
        return None
...
def ejecutar_portal_iva(usuario, periodo, tipo_operacion):
    ...
    try:
        ...
        print(f"❌ Error al importar el módulo portal_iva: {e}")
    except Exception as e:
        print(f"❌ Error al ejecutar Portal IVA: {e}")
...
def ejecutar_consulta_retenciones(usuario, fecha_desde, fecha_hasta):
    ...
    try:
        ...
        print(f"❌ Error al importar el módulo consultar_retenciones: {e}")
    except Exception as e:
...
def main():
    ...
    if opcion == "1":
        ...
        if tipo not in ["N", "E"]:
            print("❌ Tipo de operación inválido. Use N o E.")
            return
